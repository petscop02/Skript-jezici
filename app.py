...
def register():
    if request.method == "GET":
        ...
        if email == '' or username == '' or password == '' or password2 == '': #ukoliko je korisnik uneo nesto, proveravamo sve redom (odgovarajuca duzina lozinki, da li su iste lozinke, da li korisnik vec postoji)
            ...
            db.execute("INSERT INTO users(email,username,password,choice,why) VALUES (?,?,?,?,?)",email,username,hash_password,"not chosen yet",":(")
            db.execute("INSERT INTO history(username,choice,why,past) VALUES(?,?,?,?)",username,"not chosen yet",":(",0)
            all_history = db.execute("SELECT * FROM history")
            j = 0
            for i in all_history:
                j += 1
...
def choose():
    if request.method == "GET":
        ...
        db.execute("UPDATE users SET choice = ?,why = ? WHERE id = ?",choice.lower(),why,usr_id) #updatovanje objekta posto je default "not chosen yet"
        db.execute("UPDATE history SET past = ?,choice = ?, why = ? WHERE username = ?",new_past,choice,why,session["user_name"])
